...
lipo -create "$BUILD/TomatoFocus-arm64" "$BUILD/TomatoFocus-x86_64" -output "$CONTENTS/MacOS/Tommi"
...
iconutil --convert icns "$ICONSET" --output "$CONTENTS/Resources/Tommi.icns"
cat > "$CONTENTS/Info.plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
<key>CFBundleExecutable</key><string>Tommi</string>
<key>CFBundleIdentifier</key><string>com.dante9k.tomatofocus</string>
<key>CFBundleName</key><string>Tommi</string>
<key>CFBundleDisplayName</key><string>Tommi</string>
<key>CFBundleShortVersionString</key><string>$VERSION</string>
<key>CFBundleVersion</key><string>$VERSION</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleIconFile</key><string>Tommi</string>
...
lipo "$CONTENTS/MacOS/Tommi" -verify_arch arm64 x86_64
"$CONTENTS/MacOS/Tommi" --verify-ui "$BUILD/verification"
...
PY
"$CONTENTS/MacOS/Tommi" --verify-ui "$BUILD/verification/resume"
"$CONTENTS/MacOS/Tommi" --verify-ui "$BUILD/verification/overdue"
if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
    "$CONTENTS/MacOS/Tommi" --verify-ui "$BUILD/verification/login-service" --verify-login-service
fi
...
NAME="Tommi-$VERSION-macos-universal"
STAGE="$BUILD/$NAME"
mkdir -p "$STAGE"
...
python3 scripts/check-macos-package.py "$STAGE/Tommi.app" "$VERSION" --container
ditto -c -k --sequesterRsrc --keepParent "$STAGE" "$DIST/$NAME.zip"
ln -sfn /Applications "$STAGE/Applications"
hdiutil create -volname "Tommi" -srcfolder "$STAGE" -ov -format UDZO "$DIST/$NAME.dmg"
